...
namespace ABitLessCeremony.Features.Carts.Domain;
...
public abstract record AddItemToCartError
{
    ...
    // this shouldn't be here, it's only here because the command handler needs it to return the info to the API
    // this is maybe a smell, that we should get rid of the command handler indirection 🤔 
    public sealed record CartDoesNotExist(CartId CartId) : AddItemToCartError;
